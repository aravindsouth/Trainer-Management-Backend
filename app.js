...
app.get("/", (req, res) => {
  res.send(
    "<h2>ICT Trainer Management System</h2><p>FSD Project group 4</p><p>Backend</p>"
  );
  // console.log(req);
});
...
app.get(
  "/trainer-profile/:trainer_email",
  verifyTrainerToken,
  function (req, res) {
    trainerData.findOne(
      { email: req.params.trainer_email },
      (error, trainer) => {
        if (!error) {
...
app.put("/set-employment-type", function (req, res) {
  ...
  sendEmail(trainer);
  res
    .json({ status: true, reply: "employment updated email sent" })
    .status(200);
});
app.listen(PORT, () => {
